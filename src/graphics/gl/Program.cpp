...
struct Shader {
	Shader(GLenum type, const std::string &filename, const std::string &defines) {
		...
		RefCountedPtr<FileSystem::FileData> dataCode = FileSystem::gameDataFiles.ReadFile("shaders/data.glsl");
		assert(dataCode);
		RefCountedPtr<FileSystem::FileData> logzCode = FileSystem::gameDataFiles.ReadFile("shaders/logz.glsl");
		assert(logzCode);
		RefCountedPtr<FileSystem::FileData> libsCode = FileSystem::gameDataFiles.ReadFile("shaders/lib.glsl");
		assert(libsCode);
		...
		if (s_bDumpShaderSource) {
			...
			const char SHADER_PIGL_OUT_DIR_NAME[] = "shaders";
			FileSystem::userFiles.MakeDirectory(SHADER_OUT_DIR_NAME);
			FileSystem::userFiles.MakeDirectory(SHADER_PIGL_OUT_DIR_NAME);
...
{
	const std::string filename = std::string("shaders/") + name;
